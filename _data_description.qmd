...
The data comes from Table 1 in @Muller_Algavi_Borenstein_2022. Table 1 describes the curated gut {{< glossary "Microbiome" display="microbiome" >}}-{{< glossary "Metabolome" display="metabolome" >}} datasets that this study aggregated in order to curate a resource for the research community. This table is a 14 row 7 column table with columns showing

* the name of each dataset
* which reference in the study relates to that data
* a brief description of the {{< glossary "Cohort" display="cohort" >}} within that study
* the number of samples with paired fecal microbiome-metabolome data within that cohort
* whether the study contained {{< glossary "Longitudinal" display="longitudinal" >}} data
* the number of {{< glossary "HMDB" >}} annotated compounds
* the number of {{< glossary "KEGG" >}} annotated compounds

Each row in the table is providing data for that source dataset itself. Figure 1 in @Muller_Algavi_Borenstein_2022 provides more information on the overlap between datasets (e.g., in how many datasets were observed HMDB annotated {{< glossary "Metabolites" display="metabolites" >}} present).
