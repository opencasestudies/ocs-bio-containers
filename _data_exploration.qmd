...
{{< include resources/dockerfiles/starting/Dockerfile >}}
```
...
#| echo: false
#| results: asis
cat('```{.docker filename="Dockerfile"}', 
    readLines("resources/dockerfiles/starting/Dockerfile"),
    '```', 
    sep = "\n"
)
```
...
Side quest: Discuss how mounting data/scripts allows for version controlled files and that while editing the files in the file tab is possible without mounting a volume, it's not best pracitces (though it may not always be possible to mount files depending on security settings on a system)
