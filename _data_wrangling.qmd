...
If you have been following along but stopped, the scripts we are using are set up to both save and import the data to/from our mounted volume (the `ocs-bio-containers-scaffold-main` directory). Therefore in between sections, you should be able to pause, stop, or even delete a {{< glossary "Container" display="container" >}} and either unpause, re-start, or re-create the container (Step 3b from the Data Import section). 
...
1. {{< glossary "Build" >}} the modified Dockerfile to be an image (will need to do this using a {{< glossary "Command Line Interface (CLI)" display = "command line interface" >}})
