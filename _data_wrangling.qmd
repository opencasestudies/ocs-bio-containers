...
If you have been following along but stopped, the scripts we are using are set up to both save and import the data to/from our {{< glossary "Mount" display="mounted" >}} {{< glossary "Volume" display="volume" >}} (the `ocs-bio-containers-scaffold-main` directory). Therefore in between sections, you should be able to pause, stop, or even delete a {{< glossary "Container" display="container" >}} and either unpause, re-start, or re-create the container (Step 3b from the Data Import -- Container Infrastructure section). 
...
* Type `mkdir -p data/imported` and press the return / enter key.
    * This will make the "imported" directory under the "data" directory (and create the "data" directory if it doesn't already exist).
...
```{r, echo = FALSE, fig.align = 'center', out.width = "100%", fig.alt = "The directory imported (under the data directory) is starred to show that this directory needs to be created."}
ottrpal::include_slide("https://docs.google.com/presentation/d/1EFnuBBFlqjod7S1OlOoj_9-29nPO3ZhL6iJ-Pf8xzpg/edit?slide=id.g3fb50cd5a7c_0_291#slide=id.g3fb50cd5a7c_0_291")
```
...
Lines 1-5 of the script are setup lines including the script's shebang in line 1 and loading necessary packages in lines 3-5. The `here` package will be used to build relative input and output file paths on lines 7, 44, 50, and 53. The `tidyverse` is used to pipe the data from function to function (with `%>%`) and for functions like `rename` from the package `dplyr`, [`mutate` from `dplyr` (to create new columns)](https://dplyr.tidyverse.org/reference/mutate.html), `filter` from `dplyr`, and `select` from `dplyr` throughout the script. The `readr` package will be used to write an output `csv` file on line 52.
...
* Lines 11-16 rename the data frame columns so they are descriptive, reflecting the data they contain. This uses the [function `rename()`](https://dplyr.tidyverse.org/reference/rename.html)
* Lines 17-22, 32 extract the numbers (which represent the number of paired samples) that were appended to the ends of the cohort descriptions from the `cohort_description_num_paired_samples` column, storing them in their own column `num_paired_samples`. This uses the [function `str_extract()`](https://stringr.tidyverse.org/reference/str_extract.html) from the package `stringr` and a regular expression for the "pattern" to extract.
    * `\\d+$` is a [regular expression (regex)](https://r4ds.hadley.nz/regexps.html) that is used to match a pattern -- in this case all digits / numbers at the end of the string
        * `\d` matches any digit.
        * `+` is a quantifier and means 1 or more matches (digits in this case) will be included.
        * `$` is an anchor and means a pattern at the end of the string.
        * the starting backslash (`\`) is an escape character, necessary to escape the "matches any digit" part of the regular expression so R evaluates it. 
* Lines 17, 23-26, 32 remove the numbers that we just extracted from the `cohort_description_num_paired_samples` column, creating an intermediate text column (named `cohort_description_text`) that only contains the cohort descriptions (though they are still incomplete for a given row because they are multi-line). This uses the same regular expression that we used in the previous step and the [function `str_remove()`](https://stringr.tidyverse.org/reference/str_remove.html) from `stringr`.
* Lines 17, 27-31, 32 sets up a [`case_when`](https://dplyr.tidyverse.org/reference/case-and-replace-when.html) that evaluates whether the next value/row in the dataset name column is an NA value. 
    * This utilizes the [`lead()` function](https://dplyr.tidyverse.org/reference/lead-lag.html) from `dplyr` to look for the next value. 
...
* Line 33 is a filter to remove the rows where the dataset name is NA because we've already appended the important information from those rows to the previous rows and we no longer need them. Specifically, the [`filter()` function](https://dplyr.tidyverse.org/reference/filter.html) will only keep rows where the dataset name is not NA.
* Lines 34 - 36 use the [`select()` function](https://dplyr.tidyverse.org/reference/select.html) to select all but (because of the exclamation point (`!`)) the following columns (removing them):
...
* Verify which directory you are in by typing `pwd` (which stand for "print working directory") and pressing the enter / return key.
...
1. Use the [`cumsum()` function](https://rdrr.io/r/base/cumsum.html) to assign a group ID to the rows.
2. Use the [`group_by()` function](https://dplyr.tidyverse.org/reference/group_by.html) to group the data by that group ID.
3. With the [`summarize()` function](https://dplyr.tidyverse.org/reference/summarise.html), [`paste()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/paste) the cohort descriptions within each group together.
...
1. Add a newline after line 21 (`p` comes after `h` but before `r` alphabetically).
2. Add `patchwork` to line 22, indented the same way that the rest of the packages in that section are.
3. Add a space and a backslash (`\`) after the name of the package (to the end of the new line).
...
Note that another alternative is that you could create a completely new `Dockerfile`, use the original image as your base image (`FROM bioocs/ocs-bio-containers-base:main@[digest]`), and then install the single R package (in essence using a [reusable stage](https://docs.docker.com/build/building/best-practices/#create-reusable-stages)).
