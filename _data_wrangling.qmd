...
We'll need to add this code to our `02_wrangle.R` file using the same steps as before:

* navigate to the files tab
* right click on the `02_wrangle.R` script and click edit
* copy paste the following code into the script
* click save
* navigate to the exec tab
* use `02_wrangle.R` -- adjusting based on script location and learner's file location.

```{r, eval = FALSE, filename="02_wrangle.R"}
library(here)
library(tidyverse)
...
table1_extract <- raw_table %>%
    ...
    mutate(num_paired_samples =                                                  #<2>
              as.numeric(                                                        #<2>
                str_extract(cohort_description_num_paired_samples,               #<2>
                            "\\d+$"                                              #<2>
                           )                                                     #<2>
              ),                                                                 #<2>
           cohort_description_text =
              str_remove(cohort_description_num_paired_samples,                  #<3>
                         "\\d+$"                                                 #<3>
                        ),                                                       #<3>
           cohort_description =                                                  #<4>
            case_when(lead(is.na(dataset_name),                                  #<4>
                           default = FALSE) ~                                    #<4>
