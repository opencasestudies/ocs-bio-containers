...
This case study specifically uses the technology {{< glossary "Docker" >}} to {{< glossary "Build" display="build" >}} {{< glossary "Container" display="containers" >}} and run analyses within these isolated environments, on a local machine.
...
In addition, the harmonization of aggregated data requires careful consideration to ensure that data from multiple, different sources is both compatible and comparable [@Cheng_etal_2024]. Steps involved in {{< glossary "Data harmonization" display ="data harmonization">}} include mapping related variables across datasets, adjusting scales, units, or formats for proper comparison, and reducing bias due to differences in data collection. Harmonization is not performed within this case study. Typically, {{< glossary "Meta-analysis" display ="meta-analyses">}} are synthesizing summarized information rather than aggregating the underlying, raw data [@Cheng_etal_2024]. If using this curated resource to aggregate the underlying data, carefully consider what data harmonization may be appropriate [@Nan_etal_2022].
