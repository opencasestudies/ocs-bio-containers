...
1. When defining and sharing a container image, **make sure to never include {{< glossary "Personally Identifiable Information" >}} (PII) or {{< glossary "Protected Health Information" >}} (PHI)**.
  * Even if you are sharing a {{< glossary "Dockerfile" >}} or {{< glossary "Image" display="image" >}} using a private repository or in private communication within a team setting, do not include PII or PHI.
...
2. {{< glossary "Container" >}} images should be minimal and non-interactive (e.g., able to install without user interaction).
...
4. While images provide a static snapshot of a {{< glossary "Computing environment" display="computing environment" >}}, the security or health of a container isn't necessarily static and may need to be maintained over time.
5. Only use specific technology containers within the appropriate contexts (e.g., do not use {{< glossary "Docker >}} on an {{< glossary "Institutional high performance computing (HPC)" display="institutional HPC" >}} unless explicitly allowed/supported by the institution).
