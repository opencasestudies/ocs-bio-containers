...
```{r, echo = FALSE, fig.align = 'center', out.width = "100%", fig.alt = "Images are a frozen snapshot of what the computing environment will be in a container or what will be in the running container, while a container is the isolated, specified computing environment that you can work in. This is like the Mary Poppins scene where Mary Poppins, Bert, and the kids are looking at chalk drawings (images) and jump into one which has its own isolated environment (container)."}
...
```{r, echo = FALSE, fig.align = 'center', out.width = "100%", fig.alt = "Dockerfiles are the starting point in the containerization process "}
...
```{r, echo = FALSE, fig.align = 'center', out.width = "100%", fig.alt = "Images are layered and built by processing the sequential commands in a Dockerfile, stacking each layer on the previous layer according to the commands in the Dockerfile."}
...
::::
::::{.inner_block}
An image that is pulled from a registry will match the image that was pushed to the registry. If after some time (after pushing the image to the registry) an image is built from the same `Dockerfile` that was used to build the original image (the one that was pushed to the registry), these images may no longer match. After some time, rebuilding an image later using the same Dockerfile may yield a different result because dependencies change over time/may no longer be available and base images using "latest" tags can also change over time. Therefore unless versions are explicitly controlled within the `Dockerfile` ([see this blog for more information](https://oneuptime.com/blog/post/2026-02-08-how-to-pin-package-versions-in-dockerfiles-for-reproducible-builds/view)), rebuilt images may not match images on a registry even if they originate from the same Dockerfile. Also, you may be using different options or parameters to build the Docker image than were originally used. Small changes due to differing build options and time passing can impact how reproducible a container actually is in practice and is a reason why Images are typically stored in registries to share rather than sharing a `Dockerfile` (which may or may not version pin all software). 
::::
:::
...
```{r, echo = FALSE, fig.align = 'center', out.width = "100%", fig.alt = "Volumes have to be mounted when running an image to have access to outside files such as scripts or data."}
...
Containers are especially useful in the following cases because of their portability and consistency:
...
* If a team wants to both replicate and/or extend an analysis.
* If there are many dependencies that could cause nightmares.

Especially in those cases, containers are extremely useful for, but not necessary for reproducibility, extensibility, and replicability in scientific research.

```{r, fig.align = 'center', out.width = "100%", echo = FALSE, fig.alt = 'Containers promote reproducibility, extensibility, and replicability of scientific research allowing collaborative teams or other researchers to apply the same software and methods to data in a more reproducible way. Containers are not necessary for but support a new researcher using the same data and the same process to reproduce the same result. Containers are not necessary for but support a new researcher using the same data and expanding an existing process to produce an extended result. Containers are not necessary for but support a new researcher using new data with an existing process to replicate a result. Containers do this by providing consistent and portable computing environments.'}
