...
Picking the right base image when writing a Dockerfile requires a balance: finding one that has the packages/ecosystem you need while not including too much extraneous stuff you don't need (e.g., compability vs size).
...
If your instiution has strict security configurations set on your machine (possibly due to sensitive data), you may not be able to mount volumes when running a Docker container. This is because Docker as a tool requires root-level permissions and this goes against the [principle of least privilege](https://hutchdatascience.org/Ethical_Data_Handling_for_Cancer_Research/data-security.html#the-principal-of-least-privilege), an important consideration for data management. 
...
* Mounting a volume: bringing LEGOs that you have outide of the set into the act of putting the set together to modify it or make it your own. 
...
* Sandboxed software evaluation: Due to the isolation of the container environment, containers provide an isolated sandbox where software can be tested or evaluated without risking the security of the rest of the system or being impacted by other software.
