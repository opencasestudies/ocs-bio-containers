...
Before we get into fetching {{< glossary "Image" display="images" >}}, creating {{< glossary "Container" display="containers" >}}, and running analyses, we want to describe the R packages that we'll use in this case study, or that are used within the analysis scripts for the project with Collab O. Rator.
...
[readr](https://cran.r-project.org/web/packages/readr/refman/readr.html){target="_blank"} | Read and write rectangular data (like `.csv`, `.tsv`, etc.)
...
[Loading the `tidyverse` loads several specialized packages to support data wrangling](https://tidyverse.org/packages/) such as `stringr` (used to work with strings) and `dplyr` (used to wrangle, transform, and summarize tabular data like data frames). You can think of the `tidyverse` as providing a universe of related R packages. Rather than using `library(...)` multiple times to call each package, we can call `library(tidyverse)` once to load all of them. You may still want to load a package explicitly even if it's packaged within the tidyverse for greater clarity. This [posit forum thread](https://forum.posit.co/t/teaching-install-load-packages-individually-or-use-tidyverse-package/71) provides some relevant points about the topic. Also note that the `%>%` (pipe) is loaded thanks to the `tidyverse`, `dplyr`, or `magrittr` packages. Load any of those packages and you'll have access to it.
...
<details><summary>If you want to learn more about the R code used throughout...</summary>

You can use this Data Carpentry tutorial to explore how various `tidyverse` or `dplyr` commands are used to wrangle data: [https://datacarpentry.github.io/R-ecology-lesson/instructor/03-dplyr.html](https://datacarpentry.github.io/R-ecology-lesson/instructor/03-dplyr.html)
